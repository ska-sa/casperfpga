...
class QdrJack(Memory):
    ...
    def from_device_info(cls, parent, device_name, device_info, memorymap_dict):
        ...
        if ctrlreg_address == -1 or ctrlreg_length == -1:
            raise RuntimeError('Could not find ctrl reg  address or length '
                               'for Qdr %s' % device_name)
    ...
    def qdr_reset(self):
        ...
        """
        self.parent.write_int(self.control_mem, 1, blindwrite=True,
                              word_offset=0)
        self.parent.write_int(self.control_mem, 0, blindwrite=True,
                              word_offset=0)
    ...
    def qdr_delay_out_step(self, bitmask, step):
        ...
        """
        if step > 0:
            self.parent.write_int(self.control_mem, 0xffffffff,
                                  blindwrite=True, word_offset=7)
        elif step < 0:
            self.parent.write_int(self.control_mem, 0,
                                  blindwrite=True, word_offset=7)
        else:
            return
        for i in range(abs(step)):
            self.parent.write_int(self.control_mem, 0,
                                  blindwrite=True, word_offset=6)
            self.parent.write_int(self.control_mem, 0,
                                  blindwrite=True, word_offset=5)
            self.parent.write_int(self.control_mem, 0xffffffff & bitmask,
                                  blindwrite=True, word_offset=6)
            self.parent.write_int(self.control_mem,
                                  (0xf & (bitmask >> 32)) << 4,
                                  blindwrite=True, word_offset=5)
    ...
    def qdr_delay_clk_step(self, step):
        ...
        for i in range(abs(step)):
            self.parent.write_int(self.control_mem, 0,
                                  blindwrite=True, word_offset=5)
            self.parent.write_int(self.control_mem, 1 << 8,
                                  blindwrite=True, word_offset=5)
