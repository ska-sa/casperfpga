...
class Bitsnap(object):
	...
	def __init__(self, interface, block_name, bitwidth=64, adrwidth=10, ignore_trig=False, ignore_we=True):
		...
		self.ctrl = WishBoneDevice(interface, block_name+'_ss_ctrl')
		self.bram = WishBoneDevice(interface, block_name+'_ss_bram')
		self.stat = WishBoneDevice(interface, block_name+'_ss_status')
