...
class sSdramReconfigureReq(command):
	def __init__(self, commandID, seqNum, OutputMode, ClearSdram, FinishedWriting, AboutToBoot, DoReboot, ResetSdramReadAddress, ClearEthernetStats, EnableDegbugSdramReadMode, DoSdramAsyncRead, DoContinuityTest, ContinuityTestOutputLow, ContinuityTestOutputHigh):
		...
		self.ResetSdramReadAddress = self.packet2BytePacker(ResetSdramReadAddress)
		self.ClearEthernetStats = self.packet2BytePacker(ClearEthernetStats)
		self.EnableDebugSdramReadMode = self.packet2BytePacker(EnableDegbugSdramReadMode)
		...
		self.ContinuityTestOutputLow = self.packet2BytePacker(ContinuityTestOutputLow)
		self.ContinuityTestOutputHigh = self.packet2BytePacker(ContinuityTestOutputHigh)
...
class sGetEmbeddedSoftwareVersionResp(command):
	def __init__(self, commandID, seqNum, VersionMajor, VersionMinor, QSFPBootloaderVersionMajor, QSFPBootloaderVersionMinor, Padding):
		self.__dict__['_odict'] = odict()
		self.Header = sCommandHeader(commandID, seqNum,False)
